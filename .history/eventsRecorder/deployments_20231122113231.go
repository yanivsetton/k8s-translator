...
func serveWs(w http.ResponseWriter, r *http.Request, clientset *kubernetes.Clientset) {
	...
	watchInterface, err := clientset.CoreV1().Events("").Watch(context.Background(), metav1.ListOptions{
		FieldSelector: fields.OneTermEqualSelector("involvedObject.kind", "Deployment").String(),
	})
	if err != nil {
		fmt.Printf("Error watching deployment events: %v\n", err)
		return
	}
	...
	for event := range watchInterface.ResultChan() {
		...
		fmt.Println("Deployment event received:", string(jsonEvent))
