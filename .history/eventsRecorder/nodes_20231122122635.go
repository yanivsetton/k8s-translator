...
func serveWs(w http.ResponseWriter, r *http.Request, clientset *kubernetes.Clientset) {
	...
	if err != nil {
		fmt.Printf("Error watching node events: %v\n", err)
		return
	}

	fmt.Println("Connected to WebSocket client for Node events")
	for event := range watchInterface.ResultChan() {
		...
		fmt.Println("Node event received:", string(jsonEvent))
	...
	fmt.Println("Disconnected from WebSocket client for Node events")
}
...
func main() {
	...
	fmt.Println("WebSocket server for Node events started on :8080")
