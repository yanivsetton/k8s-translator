...
func serveWs(w http.ResponseWriter, r *http.Request, clientset *kubernetes.Clientset) {
	...
	if err != nil {
		fmt.Printf("Error watching Service events: %v\n", err)
		return
	}

	fmt.Println("Connected to WebSocket client for Service events")
	for event := range watchInterface.ResultChan() {
		...
		fmt.Println("Service event received:")
	...
	fmt.Println("Disconnected from WebSocket client for Service events")
}
...
func main() {
	...
	fmt.Println("WebSocket server for Service events started on :7012")
	if err := http.ListenAndServe(":7012", nil); err != nil {
