...
func serveWs(w http.ResponseWriter, r *http.Request, clientset *kubernetes.Clientset) {
	...
	watchInterface, err := clientset.BatchV1().Jobs("").Watch(context.Background(), metav1.ListOptions{
		FieldSelector: combinedSelector, // Use the combined selector as defined in your code
	})
	if err != nil {
		fmt.Printf("Error watching Job and CronJob events: %v\n", err)
		return
	}
	...
	for event := range watchInterface.ResultChan() {
		...
		fmt.Println("Job and CronJob event received:", string(jsonEvent))
